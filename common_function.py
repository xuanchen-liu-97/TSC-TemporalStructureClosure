...
def spin_ops(local_dim, normalized=True):
    ...
    if not isinstance(local_dim, (int, np.integer)) or local_dim < 2:
        raise ValueError("local_dim must be an integer >= 2")
    s = (local_dim - 1) / 2
    m = np.arange(s, -s - 1, -1)
...
def unitary_branch_distance(U):
    """
    Distance of the closest principal eigenphase to the branch cut at +/- pi.

    Returns
    -------
    distance : float
        min_j (pi - |theta_j|), where exp(i theta_j) are eigenvalues of U.
    max_abs_phase : float
        max_j |theta_j|.
    """
    phases = np.angle(np.linalg.eigvals(U))
    max_abs_phase = float(np.max(np.abs(phases)))
    return float(np.pi - max_abs_phase), max_abs_phase


def protocol_log_eig(
    hamiltonians,
    pulse_areas,
    branch_guard=True,
    branch_margin=0.98,
):
    """Protocol logarithm with an eig-based principal-branch guard."""
    return log_unitary(
        protocol_unitary(hamiltonians, pulse_areas),
...
def richardson_component(
    hamiltonians,
    degrees,
    step,
    log_fn=None,
    leading_error_order=2,
):
    """
    Richardson-extrapolated multidegree coefficient.

    Assumes
        C(h) = C + a h^p + higher-order terms,
    with p = leading_error_order (normally p=2 for the centered stencils).

    Returns
    -------
    extrapolated, coarse, fine, relative_coarse_fine
    """
    if leading_error_order <= 0:
        raise ValueError("leading_error_order must be positive")

    coarse = multidegree_component(
        hamiltonians, degrees, step, log_fn
    )
    fine = multidegree_component(
        hamiltonians, degrees, step / 2, log_fn
    )

    factor = 2 ** leading_error_order
    extrapolated = (factor * fine - coarse) / (factor - 1)

    relative_coarse_fine = (
        np.linalg.norm(coarse - fine)
        / max(np.linalg.norm(extrapolated), 1e-300)
    )

    return extrapolated, coarse, fine, relative_coarse_fine
...
def palindrome_lists(hamiltonians, areas):
    ...
    if len(hamiltonians) == 0:
        raise ValueError("hamiltonians must be non-empty")
    if len(hamiltonians) != len(areas):
        raise ValueError("hamiltonians and areas must have the same length")
...
def group_commutator_unitary(H1, H2, H3, eps):
    """
    Nested group-commutator witness W = U1 V U1^{-1} V^{-1}, V = U2 U3 U2^{-1} U3^{-1}. This implementation contains 10 elementary exponentials,
    ...
    return U(H1, eps) @ V @ U(H1, -eps) @ V.conj().T
...
__all__ = [
    ...
    "channel_basis",
    "project_onto_channels",
    "random_xyz_couplings",
