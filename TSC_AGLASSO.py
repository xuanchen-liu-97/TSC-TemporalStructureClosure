...
class _AdaptiveGroupLasso:
    ...
    def _initial_ols(self, Theta: Array, Y: Array) -> Array:
    ...
    def _lambda_max(self, Theta: Array, Y: Array, weights) -> float:
        grad0 = -(Theta.T @ Y) / Theta.shape[0]
        grad0 = grad0.copy()
        grad0[~self.valid_mask] = 0.0
    ...
    def _prox(self, B: Array, scale: float, weights) -> Array:
        out = B.copy()
        out[~self.valid_mask] = 0.0
        flat = out.ravel(order="C")
        for s, idx in self.groups.items():
            vec = flat[idx]
            ...
            if norm <= threshold:
                ...
                flat[idx] *= 1.0 - threshold / norm
        return flat.reshape(out.shape, order="C")

    def _solve_one_lambda(self, Theta, Y, lam, weights, B_start, step_size):
        B = B_start.copy()
        ...
        for iteration in range(1, self.max_iter + 1):
            gradient = (Theta.T @ (Theta @ B - Y)) / Theta.shape[0]
            gradient[~self.valid_mask] = 0.0
            proposal = B - step_size * gradient
            proposal[~self.valid_mask] = 0.0
            B_new = self._prox(proposal, step_size * lam, weights)
    ...
    def _active_supports(self, B: Array):
        flat = B.ravel(order="C")
        active = [
            s for s, idx in self.groups.items()
    ...
    def fit(self, Theta_train, Y_train, Theta_test=None, Y_test=None):
        B_init = self._initial_ols(Theta_train, Y_train)
        ...
        spectral_norm = float(np.linalg.norm(Theta_train, ord=2))
        lipschitz = spectral_norm ** 2 / Theta_train.shape[0]
        ...
        if self.verbose:
            print(f"Starting Adaptive Group LASSO path for {self.label} ({self.n_lambdas} lambdas)...")

        for path_index, lam in enumerate(lambda_path):
            t0 = time.perf_counter()
            B_pen, info = self._solve_one_lambda(
                Theta_train, Y_train, float(lam), weights, B_warm, step_size
            )
            B_warm = B_pen
            supports = self._active_supports(B_pen)
            ...
            if self.verbose and (
                ...
                print(
                    ...
                    f"iter={info['iterations']:4d} | elapsed={time.perf_counter() - start:.1f}s"
                )
...
class TSCInference:
    ...
    The remaining estimator settings are frozen to the N=8 validated v1
    protocol developed in the notebook.
    """
    ...
    def __init__(
        ...
        if self.temporal_order != 1:
            raise NotImplementedError(
                "TSCInference v1 currently supports temporal_order=1 only "
