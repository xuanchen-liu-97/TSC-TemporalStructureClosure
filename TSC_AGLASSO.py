...
- high-recall AGLASSO screening followed by contribution-based pruning
...
v3.5 keeps the Adaptive-Ridge + KKT working-set Group-LASSO core and the
two-stage F^(1) screening/pruning architecture from v3.4, but replaces the
predeclared logarithmic lambda grid with KKT-event-driven continuation. After
each solved lambda, omitted-group gradients estimate the next structural-entry
scale q_g = ||grad_g||/w_g. The next lambda combines a geometric safeguard with
a jump toward the strongest omitted-group entry event, allowing the path to
skip long support plateaus without benchmark-specific lambda_min tuning. A
very deep lambda ratio remains only as an emergency numerical safety floor.
...
_IMPLEMENTATION_VERSION = "v3.5"
_F1_PARTIAL_CHECKPOINT_FORMAT_VERSION = 4
...
class _AdaptiveGroupLasso:
    ...
    def __init__(
        ...
        # v3.5 KKT-event-driven lambda continuation. `lambda_min_ratio` is
    ...
    def _next_inactive_entry_score(
        ...
        admissible for entry. v3.5 uses max_g q_g as a data-driven guide for
    ...
    def fit(
        ...
        *,
        ...
        v3.5 keeps the v3.4 penalized core and two-stage F1 support
        certification, but makes lambda continuation data-adaptive.
        ...
        4. The adaptive path stops at an exact held-out numerical floor, after
           genuine validation degradation with support growth, at a hard
           maximum number of evaluations, or at the emergency safety floor.
        5. Optional local refinement is centered on an interior minimum
           validation-loss adaptive point, never on a BIC minimum.
        6. With pruning enabled, the minimum-validation AGLASSO support becomes
           a high-recall screening set. Contribution-ranked nested supports are
           refit and validated; the sparsest validation-equivalent support is
           retained. Without pruning, the one-SE lambda rule is preserved.
        ...
        if len(refine_path) > 0:
            ...
            print(
                "  refinement skipped: minimum search-path validation loss lies "
                "on an evaluated path boundary."
            )
        ...
        # ------------------------------------------------------------
        # Stage 3: screening -> optional contribution pruning.
        # ------------------------------------------------------------
        if self.enable_pruning:
        ...
        result = GeneratorFitResult(
            ...
            pruning_applied=bool(self.enable_pruning),
            pruning_threshold=float(pruning_threshold),
            pruning_score_method=(
                "fit-field-contribution" if self.enable_pruning else "none"
            ),
...
class TSCInference:
    ...
    The remaining estimator settings are internal/frozen. v3.5 keeps the
    ...
    omitted structural groups. F^(1) then treats the minimum-validation
    AGLASSO solution as a high-recall screening set and applies
    contribution-ranked validation pruning. BIC remains diagnostic only.
    """
    ...
    # v3.5 KKT-event-driven lambda continuation. There is no benchmark-
    ...
    def __init__(
        ...
        if self.temporal_order != 1:
            raise NotImplementedError(
                "TSCInference v3.5 currently supports temporal_order=1 only "
